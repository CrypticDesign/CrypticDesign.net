...
export default function ServicesPage() {
  ...
  return <main>
    <section className="visual-hero !min-h-[520px]">
      ...
      <div className="visual-hero__content"><div className="signal-rail text-[var(--cry-accent-magenta)]" /><span className="kicker !text-[var(--cry-accent-magenta)]">Professional services</span><h1 className="display-title">One practice, from product direction through delivery.</h1><p>Choose the capability that matches the problem in front of you. Each engagement can stand alone or combine into a focused product-design partnership.</p></div>
    </section>
    <div className="shell page-stack">
      <section><div className="section-heading"><div><span className="kicker !text-[var(--cry-accent-magenta)]">Capabilities</span><h2 className="section-title">How Cryptic Design can help.</h2></div><p>Senior, systems-level support shaped around the work—not a fixed package.</p></div>
      ...
      <section className="feature-split"><div className="feature-split__content"><span className="kicker !text-[var(--cry-accent-magenta)]">Not sure where to start?</span><h2>Bring the problem. We’ll define the right engagement.</h2><p>The inquiry path captures the product context, constraints, and outcome so the first conversation can be useful.</p><div className="hero-actions"><Link href="/professional/inquiry" className="button">Start a project</Link><Link href="/professional/case-studies" className="button secondary !border-[var(--cry-accent-magenta)]">See the work</Link></div></div><div className="feature-split__image"><Image src="/images/professional-case.png" alt="Complex systems visualization" fill sizes="(max-width:900px) 100vw, 45vw" /></div></section>
    </div>
  </main>;
}
