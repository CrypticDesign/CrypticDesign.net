...
export default async function CommunityPage() {
  ...
  return (
    <main className="community-portal community-explore">
      <section className="visual-hero community-portal__hero" aria-labelledby="community-title">
        <div className="visual-hero__image"><Image src="/images/current-focus.png" alt="A luminous blue network representing connections across Cryptic Design" fill priority sizes="100vw" /></div><div className="visual-hero__wash" />
        <div className="visual-hero__content community-portal__hero-content"><div className="signal-rail text-[#ffd400]" /><span className="kicker !text-[#ffd400]">Community / Explore</span><h1 id="community-title" className="display-title">Find the signal.<br /><em>Share the journey.</em></h1><p>See what is happening across the public Cryptic Design ecosystem and find truthful entry points into groups, events, creators, releases, and future governed participation.</p><div className="hero-actions"><Link href="#happening-now" className="button home-primary-cta">Explore what is happening</Link><Link href={authenticated ? "/" : "/account/sign-in"} className="button home-secondary-cta">{authenticated ? "Return to My Home" : "Sign in to My Home"}</Link></div></div>
      ...
      <div className="shell community-portal__stack community-explore__stack">
        <section id="happening-now" aria-labelledby="happening-now-title"><div className="public-home-portal__section-label"><h2 id="happening-now-title">Happening now / Featured</h2><span>Curated public destinations</span></div><div className="public-home-portal__featured-grid">
        ...
        <section className="community-explore__activity" aria-labelledby="community-activity-title"><div><span className="kicker">Selected Community Activity</span><h2 id="community-activity-title">No governed activity stream is connected.</h2><p>Community Explore uses curated destinations instead of a generic infinite feed. No posts, reactions, attendance, member activity, or engagement metrics are fabricated.</p></div><span className="account-status-label" data-state="closed">Activity unavailable</span></section>
        {authenticated ? <section className="community-explore__continue" aria-labelledby="continue-participating-title"><div><span className="kicker !text-[#ffd400]">Continue Participating</span><h2 id="continue-participating-title">Continue from your private platform state.</h2><p>Your authenticated identity remains authoritative while moving between Community, My Home, Library, and Account.</p></div><nav aria-label="Authenticated participation shortcuts"><Link href="/">My Home <span aria-hidden="true">→</span></Link><Link href="/library">My Library <span aria-hidden="true">→</span></Link><Link href="/account">Account <span aria-hidden="true">→</span></Link></nav></section> : null}
        <section className="community-portal__code" aria-labelledby="community-code-title"><div><span className="kicker !text-[#ffd400]">Our community. Our code.</span><h2 id="community-code-title">Participation with clear boundaries.</h2></div><ul><li><strong>Be respectful</strong><span>Treat people and their work with care.</span></li><li><strong>Be inclusive</strong><span>Design for welcome, access, and belonging.</span></li><li><strong>Be creative</strong><span>Share ideas without taking ownership.</span></li><li><strong>Protect the space</strong><span>Safety and privacy outrank engagement.</span></li></ul></section>
      </div>
    </main>
  );
}
