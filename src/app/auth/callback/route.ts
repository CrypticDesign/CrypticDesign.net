...
export async function GET(request: NextRequest) {
  ...
  if (!code) return NextResponse.redirect(canonicalSiteUrl("/account/sign-in?error=confirmation"), 303);

  try {
    const session = createRequestSupabaseClient(request);
    const { data, error } = await session.client.auth.exchangeCodeForSession(code);
    if (error || !data.user) {
      return session.applyCookies(NextResponse.redirect(canonicalSiteUrl("/account/sign-in?error=confirmation"), 303));
    }
    ...
    if (profile.error || !profile.memberId) {
      ...
      return session.applyCookies(NextResponse.redirect(canonicalSiteUrl(`/account/sign-in?error=${reason}`), 303));
    }
    const destination = "/account/reset-password";
    return session.applyCookies(NextResponse.redirect(canonicalSiteUrl(destination), 303));
  } catch {
