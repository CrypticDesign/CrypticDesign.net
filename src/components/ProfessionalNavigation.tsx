...
export default function ProfessionalNavigation() {
  ...
  return <section className="entertainment-navigation professional-navigation" data-section-theme="yellow" aria-label="Explore Professional"><div className="shell entertainment-navigation__viewport"><nav className="entertainment-navigation__bar professional-navigation__bar" aria-label="Professional sections">{items.map((item) => <Link href={item.href} key={item.href} className="entertainment-navigation__item" data-theme="yellow" aria-current={active(item.href) ? "page" : undefined}><span className="entertainment-navigation__icon"><Icon name={item.icon}/></span><span className="entertainment-navigation__copy"><strong>{item.label}</strong><small>{item.description}</small></span></Link>)}</nav></div></section>;
}
