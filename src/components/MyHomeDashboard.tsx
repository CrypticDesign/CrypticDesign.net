  ...
  return (
    <main>
      <section className="visual-hero">
        ...
        <div className="visual-hero__content"><div className="signal-rail text-[#ffd400]" /><span className="kicker !text-[#ffd400]">{authenticated ? "Your space" : "Entertainment · creativity · connection"}</span><h1 className="display-title">{greeting}</h1><p>{authenticated ? "Your character, activity, saved releases, interests, and progress are all here." : "Sign in to return to your character, library, and activity."}</p><div className="hero-actions">{authenticated ? <><Link href="/account/character" className="button">♙ &nbsp; View profile</Link><Link href="/account/settings" className="button secondary">⚙ &nbsp; Account settings</Link></> : <><Link href="/account/sign-in" className="button">Sign in</Link><Link href="/account/create" className="button secondary">Account availability</Link></>}</div></div>
      </section>
