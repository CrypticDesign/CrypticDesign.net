...
export default function CommunityNavigation() {
  ...
  return (
    <section className="entertainment-navigation community-navigation" data-section-theme="periwinkle" aria-label="Explore Community">
      <div className="shell entertainment-navigation__viewport">
        <nav className="entertainment-navigation__bar community-navigation__bar" aria-label="Community sections">
          {visibleCommunityNavigationItems().map((item) => (
            <Link href={item.href} key={item.key} className="entertainment-navigation__item" data-theme="periwinkle" aria-current={isCommunityNavigationActive(pathname, item.href) ? "page" : undefined}>
