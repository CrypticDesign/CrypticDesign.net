...
test("keeps up to three recoverable variants without creating a character", () => {
  ...
  draft = store.addVariant({ draftId: draft.id, ownerAccountId: "account_1", expectedRevision: draft.revision, label: "Variant 2", recipe: { ...DEFAULT_AVATAR_RECIPE, accent: "violet" }, occurredAt: now });
  draft = store.addVariant({ draftId: draft.id, ownerAccountId: "account_1", expectedRevision: draft.revision, label: "Variant 3", recipe: { ...DEFAULT_AVATAR_RECIPE, accent: "indigo" }, occurredAt: now });
